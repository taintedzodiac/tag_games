...
(function() {
  $(document).on('click', '.voter', function(event) {
    alert("Voting is being tested... not working yet, hang tight! :)");
    return $.ajax('/votes', {
      type: 'POST',
      data: {
        game: {
    ...
    if ($response.type === 'vote') {
      if ($response.action === 'create') {
        if ($response.success) {
          ...
          row = $($newRow);
          row.attr('id', 'vote-' + $response.vote._id.$oid);
          row.append('<td>' + $response.game.title + '</td>');
          row.append('<td>' + $response.game.platform + '</td>');
          row.append('<td style="text-align: center;" width="20%"><button class="button tiny radius round delete-vote" disabled="true">Just Added</button></td>');
          row.insertAfter('#votes-table-header');
          return $('#game_title').val('');
        }
      }
    }
  });
