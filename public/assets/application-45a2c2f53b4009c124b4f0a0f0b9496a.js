...
(function() {
  $(document).on('click', '#set-time-slot', function(event) {
    return $.ajax('/games/' + $(this).data('game'), {
      type: 'PATCH',
      data: {
        game: {
...
(function() {
  ...
  $(document).on('click', '.vote-down', function(event) {
    return $.ajax('/votes/' + $(this).data('vote'), {
      type: 'DELETE'
    });
  });

  $(document).ajaxComplete(function(event, request, settings) {
    var game_id, response, vote_button, vote_count_span, vote_id;
    ...
    if (response.type === 'vote') {
      if (response.action === 'create') {
        if (response.success) {
          ...
          return vote_count_span.text(response.total_votes);
        }
